package body Fmt.Generic_Mod_Int_Argument is

   function To_Argument (X : Mod_Int_Type) return Argument_Type'Class
   is
   begin
      return Mod_Int_Argument_Type'(Value => X, others => <>);
   end To_Argument;
   ...
   is
      ...
      B : constant Mod_Int_Type := Mod_Int_Type(Self.Base);
      Y : Mod_Int_Type := Self.Value;
      L : Natural := To'Last;
   begin
      loop
